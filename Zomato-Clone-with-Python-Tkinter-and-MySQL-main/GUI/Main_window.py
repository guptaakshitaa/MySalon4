...
class ZomatoCloneApp:
    def __init__(self, root,user_id):
        ...
        self.root.title("Home window MySalon")
        ...
        Label(self.frame, text='MySalon', bg='#fed0d1', font=('arial', 40, 'normal', 'bold')).pack(pady=(0, 10))

        # This is the section of code which creates a sub-frame for the search bar and label
        self.search_frame = Frame(self.frame, bg='#fed0d1')  # Light red background for sub-frame
        self.search_frame.pack(pady=(0, 10))

        # This is the section of code which creates the label "Restaurants"
        Label(self.search_frame, text='Service', bg='#fed0d1', font=('Times New Roman', 20, 'normal','bold')).pack(side=LEFT, padx=(0, 10))
        ...
        self.order_online_button = Button(self.button_frame, text='Book Appointment', bg='#F0F8FF', font=('arial', 15, 'normal'), command=self.display_top_restaurants)
        ...
        self.you_button = Button(self.root, text='❤️YOU❤️', bg='#F0F8FF', font=('arial', 15, 'normal'), command=self.you)
        ...
        self.results_frame = Frame(self.canvas, bg='#fed0d1')  # Light red background for frame
    ...
    def display_top_restaurants(self):
        ...
        if top_restaurants:
            ...
            Label(self.results_frame, text="No restaurants found.", bg='#fed0d1', font=('arial', 12, 'normal')).pack(pady=5)
    ...
    def view_menu(self, restaurant):
        ...
        menu_window.configure(background='#fed0d1')  # Light red background for menu window
        menu_window.title(f"Order food online in {restaurant[1]}")
        menu_window.state('zoomed')

        # Heading label
        Label(menu_window, text=f"Order food online in {restaurant[1]}", bg='#fed0d1', font=('arial', 20, 'normal')).pack(pady=(10, 10))
        ...
        # Create a canvas widget and associate it with a scrollbar
        canvas = Canvas(menu_window, bg='#fed0d1')
        ...
        menu_frame = Frame(canvas, bg='#fed0d1')
    ...
    def order_history_window(self):
        ...
        order_history_win.configure(bg="#fed0d1")

        title_frame = Frame(order_history_win, bg="#fed0d1")
        title_frame.pack(fill="x", padx=10, pady=10)

        label = Label(title_frame, text="Order History", font=("arial", 30, "normal"), bg="#fed0d1")
        label.pack(side="left", padx=50)

        order_history_frame = Frame(order_history_win, bg="#fed0d1")
        order_history_frame.pack(fill="both", expand=True)

        canvas = Canvas(order_history_frame, bg="#fed0d1")
        ...
        inner_frame = Frame(canvas, bg="#fed0d1")
    ...
    def you(self):
            ...
            you_window.configure(bg='#fed0d1')
            ...
            Label(you_window, text='User Details', font=('arial', 30), bg='#fed0d1').pack(pady=20)

            frame = Frame(you_window)
            frame.pack(pady=10, padx=20)
            frame.configure(bg='#fed0d1')
            ...
            for key, value in self.user_details.items():
                ...
                detail_frame.configure(bg="#fed0d1")

                Label(detail_frame, text=f'{key}:', font=('arial', 25), bg='#fed0d1').pack(padx=40, pady=30, side=LEFT)
                Label(detail_frame, text=value, font=('arial', 25), bg='#fed0d1').pack(pady=30, side=RIGHT)
    ...
    def edit_user(self):
        ...
        edit_window.configure(bg='#fed0d1')

        frame = Frame(edit_window)
        frame.pack(pady=10, padx=20)
        frame.configure(bg='#fed0d1')

        Label(frame, text='Edit User Details', font=('arial', 30), bg='#fed0d1').pack(pady=20)
